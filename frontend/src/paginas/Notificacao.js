    ...
    useEffect(() => {
        if (data?.data?.message) {
            const projetosFiltrados = data.data.message.filter(projeto => {
                ...
                return ultimaMedida.temperatura < 20 || ultimaMedida.nivel < 30 || ultimaMedida.peso < 50 || ultimaMedida.umidade > 60;
            });
    ...
    return (
        <ListaDevices>
            ...
            {projetosComValorVermelho.length === 0 ? (
                <TextoNenhumDevice>Todas os devices estão apresentando valores dentro dos parâmetros!</TextoNenhumDevice>
            ) : (
                <ListaCards>
